...
from beapder.network.item import Item
from beapder.network.request import Request
from beapder.utils import metrics
from beapder.utils.decorators import Singleton
...
class ControlParams:
    ...
    def __init__(self):
        self.max_deal_request_count = setting.getint("MAX_DEAL_REQUEST_COUNT", 0)
        self.deal_request_count = 0
        self.max_idle_request_count = setting.getint("MAX_IDLE_REQUEST_COUNT", 0)
        self.idle_request_count = 0
...
class ParserControl(threading.Thread):
    ...
    def run(self):
        self._thread_stop = False
        max_deal_request_count = setting.getint("MAX_DEAL_REQUEST_COUNT", 0)
        deal_request_count = 0
        max_idle_request_count = setting.getint("MAX_IDLE_REQUEST_COUNT", 0)
        ...
        while not self._thread_stop:
            try:
                ...
                if not request:
                    if 0 < max_idle_request_count <= idle_request_count:
                        log.warning("当前空闲数{} 已经超过最大空闲数 > MAX_IDLE_REQUEST_COUNT={}, "
                                    "退出获取任务线程".format(idle_request_count, max_idle_request_count))
                        for parser in self._parsers:
                            parser._close_reason = "条件终止"
                        self.is_show_tip = True
                        break
                    if not self.is_show_tip:
                        log.debug("等待任务...")
                        if max_idle_request_count > 0:
                ...
                if max_deal_request_count > 0:
                    deal_request_count += 1
                    if deal_request_count > max_deal_request_count:
                        log.warning("当前任务数{} 已经超过最大任务数 > MAX_DEAL_REQUEST_COUNT={}, "
                                    "退出获取任务线程".format(idle_request_count, max_idle_request_count))
                        for parser in self._parsers:
                            parser._close_reason = "条件终止"
                        self.is_show_tip = True
    ...
    def record_download_status(self, status, spider, exc=None):
        """
        记录html等文档下载状态
        @return:
        """
        if isinstance(exc, Exception):
            metrics.emit_counter(f"{spider}:{status}", 1, classify="document", tags=dict(_type=exc.__class__.__name__))
        else:
        ...
        while not self._thread_stop:
            try:
                ...
                if not request:
                    ...
                    if 0 < const_control.max_idle_request_count:
                        const_control.idle_request_count += 1
                        if const_control.max_idle_request_count < const_control.idle_request_count:
                            log.warning("当前空闲数{} 已经超过最大空闲数 > MAX_IDLE_REQUEST_COUNT={}, "
                                        "退出获取任务线程".format(const_control.idle_request_count,
                                                          const_control.max_idle_request_count))

                            for parser in self._parsers:
                                parser.stop_spider(signal.CTRL_C_EVENT)
                ...
                self.is_show_tip = False
                for req in arg_to_iter(request):
                    self.deal_request(req)

                    if const_control.max_deal_request_count > 0:
                        const_control.deal_request_count += 1
                        if const_control.deal_request_count > const_control.max_deal_request_count:
                            log.warning("当前任务数{} 已经超过最大任务数 > MAX_DEAL_REQUEST_COUNT={}, "
                                        "退出获取任务线程".format(const_control.deal_request_count,
                                                          const_control.max_deal_request_count))

                            for parser in self._parsers:
                                log.info("stop_spider {}".format(parser.name))
                                parser.stop_spider(signal.CTRL_C_EVENT)
                            self.is_show_tip = True
                            self.stop()
                            log.info("AirControl Thread Stop")
                            return
                        else:
                            self._memory_db.add(req, True)
            ...
            if parser.name == request.parser_name:
                try:
                    ...
                    if exception_type.startswith("requests"):
                        # 记录下载失败的文档
                        self.record_download_status(
                            ParserControl.DOWNLOAD_EXCEPTION, parser.name, e
                        )

                    else:
                        # 记录解析程序异常
                        self.record_download_status(
                            ParserControl.PAESERS_EXCEPTION, parser.name, e
                        )
