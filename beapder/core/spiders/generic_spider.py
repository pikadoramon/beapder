...
from beapder.utils.generic import traverse_obj
            ...
            if rule.follow:
                ...
                for link in arg_to_iter(rule.process_links(links)):
                    if self.max_depth and depth >= self.max_depth:
                        log.debug(
                            "[%(parser)s] Ignoring link (depth > %(maxdepth)d): %(requrl)s ",
                            {'maxdepth': self.max_depth, 'requrl': link, 'parser': request.parser_name},
                        )
                        continue
                    request_kwargs = {
