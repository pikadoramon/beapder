...
from beapder.network import user_agent
from beapder.network.downloader.base import Downloader, RenderDownloader
...
from beapder.utils.generic import NULL_DEFAULT
...
class Request:
    attributes: Tuple[str, ...] = (
        "method", "retry_times", "priority", "parser_name", "callback", "filter_repeat", "auto_request", "request_sync",
        "use_session", "random_user_agent", "download_midware", "is_abandoned", "render", "render_time",
        "make_absolute_links", "meta",
    )
    ...
    def __init__(
        ...
        # 这里属于beapder定义字段
        # 这里定义trace-id
        # 定义trace-id的目的是为了进行分布式日志收集时，能够定位所关联的请求
        # 是携带的原信息
        self._trace_id = None
    ...
    def trace_id(self, value):
        if self._trace_id is None:
            ...
            raise AttributeError("Error in duplicating the setting of trace_id.")
    ...
    def _proxies_pool(self):
        if not self.__class__.proxies_pool:
            proxy_pool_path = setting.get("PROXY_POOL", "beapder.network.proxy_pool.ProxyPool")
            log.info(f"实例化代理池模块 [{proxy_pool_path}]")
            proxy_poll_obj = tools.import_cls(proxy_pool_path)
            if hasattr(proxy_poll_obj, "from_settings"):
                self.__class__.proxies_pool = proxy_poll_obj.from_settings(settings=setting)
            elif callable(proxy_poll_obj):
                self.__class__.proxies_pool = proxy_poll_obj()
            else:
    ...
    def _render_downloader(self):
        if not self.__class__.render_downloader:
            try:
                ...
                log.error('当前是渲染模式，请安装 pip install "beapder[render]"')
                os._exit(0)
            ...
            if value is not None:
                if key in self.__class__.__REQUEST_ATTRS__:
                    ...
                    if not isinstance(value, (bytes, bool, float, int, str)):
                        value = tools.dumps_json(value, indent=0)
    ...
    def make_requests_kwargs(self):
        ...
        # 代理
        proxies = self.requests_kwargs.get("proxies", NULL_DEFAULT)
        if proxies is NULL_DEFAULT and setting.PROXY_ENABLE and setting.PROXY_EXTRACT_API:
            while True:
