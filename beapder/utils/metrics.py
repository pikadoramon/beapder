...
from beapder.extensions.metrics_emitter import MetricsEmitterABC
from beapder.utils.log import log
from beapder.utils.tools import aio_wrap, ensure_float, ensure_int, import_cls
...
_emitter: MetricsEmitterABC = None
...
def init(
    ...
    emitter_cls = settings.get("BEAPDER_EXTENSION_METRIC_EMITTER")
    if emitter_cls is None:
        emitter_cls = "beapder.extensions.metrics_emitter.InfluxDBMetricsEmitter"
    _emitter_obj = import_cls(emitter_cls)
    if hasattr(_emitter_obj, "from_settings"):
        _emitter = _emitter_obj.from_settings(settings)
    elif hasattr(_emitter_obj, "init"):
        _emitter = _emitter_obj.init(settings, *args, **kwargs)
    else:
        raise ImportError("metrics[%s] Initialization failed due to the absence of 'from_settings' or 'init' instance "
                          "methods." % emitter_cls)
    _inited_pid = os.getpid()
    log.info("metrics[%s] init successfully" % emitter_cls)
