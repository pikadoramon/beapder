...
from beapder.utils.decorators import run_safe_model
...
def get_all_params(url):
    """
    >>> get_all_params("https://www.baidu.com/s?wd=beapder")
    {'wd': 'beapder'}
    """
    params_json = {}
...
def dumps_json(data, indent=4, sort_keys=False):
    ...
    try:
        if isinstance(data, str):
            data = get_json(data)
        if setting.getbool("DUMP_JSON_DISABLE_INDENT"):
            indent = None
        data = json.dumps(
            data,
            ensure_ascii=False,
...
def mkdir(path):
    try:
        if not os.path.exists(path):
            os.makedirs(path)
    except OSError as _:  # Python >2.5
        pass
...
def get_cache_path(filename, root_dir=None, local=False):
    ...
    if root_dir is None:
        if local:
            ...
            root_dir = os.path.join(os.path.expanduser("~"), ".beapder/cache")
...
def get_uuid(key1="", key2=""):
    ...
    if not key1 and not key2:
        ...
        _hash = md5(bytes(key1, "utf-8") + bytes(key2, "utf-8")).digest()
        uuid_object = uuid.UUID(bytes=_hash[:16], version=3)
