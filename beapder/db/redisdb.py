...
class RedisDB:
    ...
    def __repr__(self):
        ...
        return "<Redisdb ip_ports: {} db:{} user_pass:{}>".format(
            self._ip_ports, self._db, self._user_pass[:3] if self._user_pass else "null"
        )
    ...
    def get_connect(self):
        ...
        try:
            if not self._url:
                ...
                if len(ip_ports) > 1 and self._service_name != "single":
                    startup_nodes = []
                    for ip_port in ip_ports:
                        ip, port = ip_port.split(":")
                        startup_nodes.append({"host": ip, "port": port})

                    if self._service_name and self._service_name != "cluster":
                        ...
                        try:
                            ...
                            log.error('请安装 pip install "rediscluster"')
                            os._exit(0)
                    ...
                    ip, port = random.choice(ip_ports).split(":")
    ...
    def sadd(self, table, values):
        ...
        if isinstance(values, list):
            with self._redis.pipeline(self.transaction) as pipe:
    ...
    def zrangebyscore(self, table, priority_min, priority_max, count=None, is_pop=True):
        ...
        """
        if count:
            results = self._redis.zrangebyscore(table, priority_min, priority_max, withscores=False, start=0, num=count)
        else:
            results = self._redis.zrangebyscore(table, priority_min, priority_max, withscores=False)
        if is_pop and results:
            self._redis.zrem(table, *results)
        return results
    ...
    def zrangebyscore_increase_score(self, table, priority_min, priority_max, increase_score, count=None):
        ...
        res = self.zrangebyscore(table, priority_min, priority_max, count, False)
        with self._redis.pipeline(self.transaction) as pipe:
            pipe.multi()
            for value in res:
                pipe.zincrby(table, increase_score, value)
            pipe.execute()
        return res
    ...
    def zrangebyscore_set_score(
        ...
        res = self.zrangebyscore(table, priority_min, priority_max, count, True)
        with self._redis.pipeline(self.transaction) as pipe:
            pipe.multi()
            for value in res:
                pipe.execute_command("ZADD", table, score, value)
            pipe.execute()
        return res
    ...
    def hget(self, table, key, is_pop=False):
        ...
        result = self._redis.hget(table, key)
        if not is_pop:
            return result
        ...
        # 当前的执行并发数还不足以使用lua保证原子性
        # P.s codis无法使用register_script
        self._redis.hget(table, key)
        self._redis.hdel(table, key)
        return result
...
class ContainDB:
    ...
    def __init__(self, name, db, local_max_cached_size=100, full_op='delay', max_timeout=30):
        self.name = name
        self._db: RedisDB = db
        assert isinstance(local_max_cached_size, int) and local_max_cached_size > 0
        self.local_max_cached_size = local_max_cached_size
        self.full_op = full_op
    ...
    def add(self, item, ignore_max_size=False):
        ...
        """
        if ignore_max_size:
            self._db.lpush(self.name, item)

        elif self.local_max_cached_size is None or self.local_max_cached_size < 1:
            self._db.lpush(self.name, item)

        elif self.size() < self.local_max_cached_size:
            self._db.lpush(self.name, item)
            return

        elif self.full_op == 'delay':
            s = time.time()
            while self.size() >= self.local_max_cached_size:
                time.sleep(1)
                if time.time() - s > self.max_timeout:
                    log.warning("delay %s 数据已满. 丢弃该item %s" % (self.name, item))
                    return
            self._db.lpush(self.name, item)
        else:
            log.warning("fail %s 数据已满. 丢弃该item %s" % (self.name, item))
    ...
    def get(self):
        ...
        :return:
        """
        item = self._db.lpop(self.name)
        return item

    def empty(self):
        return self._db.lget_count(self.name) == 0

    def size(self):
        return self._db.lget_count(self.name)
