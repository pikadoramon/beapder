...
from beapder.core.handle_failed_items import HandleFailedItems
from beapder.core.handle_failed_requests import HandleFailedRequests
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        usage="usage: beapder retry [options] [args]",
    )
    parser.add_argument(
        "-r",
        "--request",
        help="重试失败的request 如 beapder retry --request <redis_key>",
        metavar="",
    )
    parser.add_argument(
        "-i", "--item", help="重试失败的item 如 beapder retry --item <redis_key>", metavar=""
    )
