...
from beapder.utils.ext import setting
...
from beapder.pipelines.mysql_pipeline import MysqlPipeline
from beapder.utils import metrics
from beapder.utils.log import log
from beapder.utils.rule import arg_to_iter

MYSQL_PIPELINE_PATH = "beapder.pipelines.mysql_pipeline.MysqlPipeline"
...
class ItemBuffer(threading.Thread):
    ...
    def load_pipelines(self):
        ...
        for pipeline_path in arg_to_iter(setting.ITEM_PIPELINES):
            log.info(f"实例化 pipeline [{pipeline_path}]")
            pipeline_object = tools.import_cls(pipeline_path)
            if hasattr(pipeline_object, "from_settings"):
                pipeline = pipeline_object.from_settings(settings=setting)
            else:
                pipeline = pipeline_object()
            if not isinstance(pipeline, BasePipeline):
                raise ValueError(f"{pipeline_path} 需继承 beapder.pipelines.BasePipeline")
    ...
    def mysql_pipeline(self):
        if not self._mysql_pipeline:
            _mysql_pipeline = tools.import_cls(MYSQL_PIPELINE_PATH)
            if hasattr(_mysql_pipeline, "from_settings"):
                self._mysql_pipeline = _mysql_pipeline.from_settings(settings=setting)
            else:
                self._mysql_pipeline = _mysql_pipeline()
