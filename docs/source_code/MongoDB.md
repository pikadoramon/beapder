...
class TestMongo(beapder.AirSpider):
    __custom_setting__ = dict(
        ITEM_PIPELINES=["beapder.pipelines.mongo_pipeline.MongoPipeline"],
        MONGO_IP="localhost",
        MONGO_PORT=27017,
        MONGO_DB="beapder",
        MONGO_USER_NAME="",
...
```python
from beapder.db.mongodb import MongoDB


db = MongoDB(
    ip="localhost", port=27017, db="beapder", user_name="beapder", user_pass="beapder123"
)
```
