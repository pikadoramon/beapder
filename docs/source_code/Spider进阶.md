...
class SpiderTest(beapder.Spider):
    ...
    def start_requests(self):
        sql = "select url from beapder_test where state = 0 limit 1000"
        result = self.db.find(sql)
        for url, in result:
            yield beapder.Request(url)
...
if __name__ == "__main__":
    spider = SpiderTest(
        redis_key="beapder:spider_test", min_task_count=100
    )
...
删除`beapder:spider_test_z_requests` 可写为 `delete_keys="*z_requests"`
...
```
def spider_test():
    spider = test_spider.TestSpider(redis_key="beapder:test_spider", batch_interval=7)
    spider.start()
...
使用：
    ...
    spider = test_spider.TestSpider(redis_key="beapder:test_spider", keep_alive=True)
    # 下发及监控任务
    spider.start_monitor_task()
    # 采集（进程常驻）
    # spider.start()
