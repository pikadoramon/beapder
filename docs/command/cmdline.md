...
命令行工具为**beapder**内置支持的，可方便快速的创建项目、爬虫、item、以及调试请求等，使用方法如下：
...
打开命令行窗口，输入beapder
    ...
    Available commands:
      create        create project、beapder、item and so on
...
可见beapder支持`create`、`shell`及`zip`三种命令

## 2. beapder create

使用beapder create 可快速创建项目、爬虫、item等，具体支持的命令可输入`beapder create -h` 查看使用帮助
    ...
    optional arguments:
      ...
      -p , --project    创建项目 如 beapder create -p <project_name>
      -s , --spider     创建爬虫 如 beapder create -s <spider_name>
      -i , --item       创建item 如 beapder create -i <table_name> 支持模糊匹配 如 beapder create -i %table_name%
      -t , --table      根据json创建表 如 beapder create -t <table_name>
      -init             创建__init__.py 如 beapder create -init
      -j, --json        创建json
      -sj, --sort_json  创建有序json
      -c, --cookies     创建cookie
      --params          解析地址中的参数
      --setting         创建全局配置文件beapder create --setting
      --host            mysql 连接地址
      --port            mysql 端口
      --username        mysql 用户名
      --password        mysql 密码
...
命令
    ...
    beapder create -p <project_name>

示例：

    beapder create -p first-project
...
若不配置setting.py, 可在命令行中指定数据库连接信息
     ...
     beapder create -i spider_data --host localhost --db beapder --username beapder --password beapder123
...
class SpiderDataItem(Item):
    """
    This class was generated by beapder.
    command: beapder create -i spider_data 1.
    """
...
命令：
    ...
    beapder create -t <table_name>

用的示例：

    > beapder create -t test
...
下载调试器，可方便测试抽取规则是否正确，输入`beapder shell -h`查看使用帮助
...
1. 以url为例，请求百度：
    ...
    请求
        ...
        > beapder shell -u https://www.baidu.com
...
1. 以curl为例，请求百度（通常用来测试post接口比较方便）
    ...
    1. 测试 输入`beapder shell --`, 粘贴刚刚复制的curl

            > beapder shell --curl 'https://www.baidu.com/' \
