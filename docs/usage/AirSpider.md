...
```python
beapder create -s air_spider_test
...
生成如下
    ...
    class AirSpiderTest(beapder.AirSpider):
...
默认生成的代码继承了beapder.AirSpider，包含 `start_requests` 及 `parser` 两个函数，含义如下：

1. beapder.AirSpider：轻量爬虫基类
2. start_requests：初始任务下发入口
3. beapder.Request：基于requests库类似，表示一个请求，支持requests所有参数，同时也可携带些自定义的参数，详情可参考[Request](source_code/Request.md)
4. parser：数据解析函数
...
开发过程中解析函数往往不止有一个，除了系统默认的parser外，还支持自定义解析函数，写法如下
    ...
    def start_requests(self):
        yield beapder.Request("https://www.baidu.com", callback=self.parser_xxx)

    def parser_xxx(self, request, response):
        print(response)

即beapder.Request支持指定callback函数，不指定时默认回调parser
...
有时我们需要把前面请求到的数据携带到下一级，写法如下
    ...
    def start_requests(self):
        yield beapder.Request("https://www.baidu.com",  xxx="我是携带的数据")

    def parse(self, request, response):
        xxx = request.xxx
        print(xxx)

直接在beapder.Request中携带即可，xxx为携带数据的key，可以随意写，只要不和beapder.Request默认参数冲突即可。默认参数参考[Request](source_code/Request.md)。可以携带任意类型的值，如字典、类等

取值：如何携带就如何取值，如上我们携带xxx， 那么`request.xxx` 可将xxx值取出，取出的值和携带的值类型一致。

## 5. 下发新任务

parser中支持下发新任务，写法与start_requests一致，只需要`yield beapder.Request`即可。示例如下：

    def parse(self, request, response):
        yield beapder.Request("url1") # 不指定callback，任务会调度默认的parser上
        yield beapder.Request("url2", callback=self.parser_detail) # 指定了callback，任务由callback指定的函数解析
...
与自定义解析函数类似，下载中间件也支持自定义，只需要在beapder.Request参数里指定个`download_midware`回调即可，写法如下：

    def start_requests(self):
        yield beapder.Request("https://www.baidu.com", download_midware=self.xxx)
...
class AirSpeedTest(beapder.AirSpider):
    def start_requests(self):
        yield beapder.Request("http://www.baidu.com")
...
import beapder
import httpx
from beapder.network.selector import Selector
...
AirSpider：https://github.com/Boris-code/beapder/blob/master/tests/air-spider/test_air_spider.py

浏览器渲染：https://github.com/Boris-code/beapder/blob/master/tests/test_rander.py
