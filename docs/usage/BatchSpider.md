...
```python
beapder create -s batch_spider_test
...
class BatchSpiderTest(beapder.BatchSpider):
...
1. redis_key：redis中存储任务等信息的key前缀，如redis_key="beapder:spider_test", 则redis中会生成如下
...
def crawl_test(args):
    spider = test_spider.TestSpider(
        redis_key="beapder:test_batch_spider",  # 分布式爬虫调度信息存储位置
...
```
    def start_requests(self, task):
        ...
        yield beapder.Request(url, task_id=id)
...
[https://github.com/Boris-code/beapder/tree/master/tests/batch-spider](https://github.com/Boris-code/beapder/tree/master/tests/batch-spider)