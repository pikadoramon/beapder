...
**feaplat**命名源于 beapder 与 platform 的缩写
...
1. 支持部署任何程序，包括不限于`beapder`、`scrapy`
...
feaplat支持对beapder爬虫的运行情况进行监控，除了数据监控和请求监控外，用户还可自定义监控内容，详情参考[自定义监控](source_code/监控打点?id=自定义监控)
...
![beapderd](http://markdown-media.oss-cn-beijing.aliyuncs.com/2021/07/23/beapderd.png)
...
- 若端口冲突，可修改.env文件，参考[常见问题](beapder_platform/question?id=修改端口)
...
- 若未成功，参考[常见问题](beapder_platform/question)
- 使用说明，参考[使用说明](beapder_platform/usage)
...
默认的爬虫镜像只打包了`beapder`、`scrapy`框架，若需要其它环境，可基于`.env`文件里的`SPIDER_IMAGE`镜像自行构建

如自定义python版本，安装常用的库等，需修改feaplat下的`beapder_dockerfile`

```
# 基于最新的版本，若需要自定义python版本，则要求beapder版本号>=2.4
FROM registry.cn-hangzhou.aliyuncs.com/beapderd/beapder:2.4
...
改好后要打包镜像，打包命令：
```
docker build -f beapder_dockerfile -t 镜像名:版本号 .
```
如
```
docker build -f beapder_dockerfile -t my_beapder:1.0 .
```

打包好后修改下 `.env`文件里的 SPIDER_IMAGE 的值即可如：
```
SPIDER_IMAGE=my_beapder:1.0
```
...
2. 若自定义了python版本，则需要添加挂载，否则feaplat上自动安装的依赖库不会保留。挂载方式：修改`docker-compose.yaml`的        SPIDER_RUN_ARGS参数。如
   ```
   SPIDER_RUN_ARGS=["--mount type=volume,source=beapder_python3.10,destination=/usr/local/python-3.10.8"]
   ```
