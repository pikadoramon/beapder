# -*- coding: utf-8 -*-
"""
Created on 2023-10-31 17:17:21
---------
@summary:
...
@click.option("--args", "-a", help="全局参数指定", type=str, multiple=True)
...
def run(runner, args):
    ...
    if hasattr(runner_object, "from_settings"):
        logger.error(f"你传入的是object, 现在from_settings进行实例化. 开始实例{method}.from_settings(settings)")
        spider_object_ = runner_object.from_settings(settings)
        logger.error(f"准备调用方法 spider_object_.start")
...
@cli.command()
@click.option("--method", "-m", help="需要执行方法路径", type=str, default="")
@click.option("--args", "-a", help="实例参数指定", type=str, multiple=True)
def execute(method, args):
    ...
    runner_object = import_cls(method)
    if hasattr(runner_object, "from_settings"):
        logger.info(f"你传入的是object, 现在from_settings进行实例化. 开始实例{method}.from_settings(settings)")
        spider_object_ = runner_object.from_settings(settings)
        logger.info(f"准备调用方法 spider_object_.start")
        spider_object_.start()
    elif callable(runner_object):
        logger.info(f"准备调用方法 {method}")
        runner_object(settings)
    else:
        logger.error(method + "应用无from_settings方法, 无法实例应用")
        sys.exit(2)
